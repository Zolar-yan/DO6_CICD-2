...
do
	./s21_cat $flag test_cat.txt > s21_cat.txt
	cat $flag test_cat.txt > cat.txt
...
do
	...
	do
		if [ $flag != $flag2 ]
		then
		  ./s21_cat $flag $flag2 test_cat.txt > s21_cat.txt
		  cat $flag $flag2 test_cat.txt > cat.txt
		  VERDICT="$(diff -s s21_cat.txt cat.txt)"
		  if [ "$VERDICT" == "Files s21_cat.txt and cat.txt are identical" ]
		    then
		      (( SUCCESS++ ))
...
do
  ...
  do
    ...
    do
      ...
      then
        ./s21_cat $flag $flag2 $flag3 test_cat.txt > s21_cat.txt
        cat $flag $flag2 $flag3 test_cat.txt > cat.txt
        VERDICT="$(diff -s s21_cat.txt cat.txt)"
	if [ "$VERDICT" == "Files s21_cat.txt and cat.txt are identical" ]
          then
            (( SUCCESS++ ))
  ...
  do
    ...
    do
      ...
      do
        ...
        then
          ./s21_cat $flag $flag2 $flag3 $flag4 test_cat.txt > s21_cat.txt
          cat $flag $flag2 $flag3 $flag4 test_cat.txt > cat.txt
          VERDICT="$(diff -s s21_cat.txt cat.txt)"
          if [ "$VERDICT" == "Files s21_cat.txt and cat.txt are identical" ]
            then
	      (( SUCCESS++ ))
